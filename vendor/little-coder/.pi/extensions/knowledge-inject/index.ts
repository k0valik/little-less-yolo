...
function dirs(): string[] {
  ...
  const result = [join(repo, "skills", "knowledge"), join(repo, "skills", "protocols")];
  // Also load from the persistent mount (user overrides).
  // PI_CODING_AGENT_DIR is always set inside the pi-less-yolo container;
  // for native little-coder usage the directory may not exist → silently skipped.
  const agentDir = process.env.PI_CODING_AGENT_DIR;
  if (agentDir && agentDir.trim().length > 0) {
    const userKnowledge = join(agentDir, "skills", "knowledge");
    if (existsSync(userKnowledge)) result.push(userKnowledge);
    const userProtocols = join(agentDir, "skills", "protocols");
    if (existsSync(userProtocols)) result.push(userProtocols);
  }
  return result;
}
