...
export function resolveOverridePath(env: NodeJS.ProcessEnv = process.env): string | undefined {
  ...
  if (xdg) {
    const candidate = join(xdg, "little-coder", "models.json");
    if (existsSync(candidate)) return candidate;
  }
  if (env.HOME) {
    const candidate = join(env.HOME, ".config", "little-coder", "models.json");
    if (existsSync(candidate)) return candidate;
  }
  // pi-less-yolo: PI_CODING_AGENT_DIR points to /pi-agent (host-mounted
  // ~/.pi/agent-little-coder). Users can drop models.json there.
  const piAgentDir = env.PI_CODING_AGENT_DIR;
  if (piAgentDir && piAgentDir.trim().length > 0) {
    const candidate = join(piAgentDir, "models.json");
    if (existsSync(candidate)) return candidate;
  }
  return undefined;
}
