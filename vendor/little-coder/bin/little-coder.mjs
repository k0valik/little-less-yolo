...
/** Resolve PI_CODING_AGENT_DIR or fall back to ~/.pi/agent */
function resolveAgentDir() {
  const envVal = process.env.PI_CODING_AGENT_DIR;
  if (envVal && envVal.trim().length > 0) {
    return envVal === "~"
      ? homedir()
      : envVal.startsWith("~/")
        ? homedir() + envVal.slice(1)
        : envVal;
  }
...
// ---- 4. Auto-discover extensions (bundled + user) ----
// Load from package root first, then from agentDir/extensions/.
// User extensions with the same name override bundled ones.
const EXT_PATHS = [
  { dir: join(pkgRoot, ".pi", "extensions"), label: "bundled" },
  { dir: join(agentDir, "extensions"),       label: "user" },
];
const extByName = new Map(); // name → path (user overwrites bundled for same name)
for (const { dir } of EXT_PATHS) {
  ...
  for (const name of readdirSync(dir).sort()) {
    ...
    try {
      if (statSync(subdir).isDirectory() && existsSync(idx)) {
        extByName.set(name, idx); // overwrites bundled if user version exists
      }
    } catch {
...
const extArgs = [];
for (const [, idx] of extByName) {
  extArgs.push("--extension", idx);
}

// ---- 5. Suppress version-check noise (both little-coder's and pi's) ----
// Set before the update check so it can consume PI_SKIP_VERSION_CHECK.
// Also consumed by pi's own version-banner later (step 7 reasserts it).
if (process.env.PI_SKIP_VERSION_CHECK === undefined) {
  process.env.PI_SKIP_VERSION_CHECK = "1";
}

// ---- 6. Update check (best-effort, blocks on TTY prompt only) ----
...
// --system-prompt    : load AGENTS.md — checks agentDir first, then pkgRoot
//
...
} else if (existsSync(join(pkgRoot, "AGENTS.md"))) {
  agentsMd = join(pkgRoot, "AGENTS.md");
}
const piArgs = [
  "--no-context-files",
  "--no-extensions",
  ...(agentsMd ? ["--system-prompt", agentsMd] : []),
  ...extArgs,
  ...userArgs,
];

// ---- 8. Force pi's global quietStartup + pin lastChangelogVersion ----
// Two non-destructive merges into <agentDir>/settings.json:
//
//   1. quietStartup: true
...
//        upstream changelog can still do with `/changelog` inside the TUI.
//
